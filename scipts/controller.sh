...
if [ $1 = "controller" ]; then
	sudo python pox.py openflow.of_01 --port=6635 forwarding.mycontroller samples.pretty_log info.packet_dump log.level 
	echo "Running controller"
elif [ $1 = "clean" ]; then
	echo "Running cleanup"
	sudo python pox.py openflow.of_01 --port=6635 forwarding.clean_up samples.pretty_log info.packet_dump log.level 
else 
