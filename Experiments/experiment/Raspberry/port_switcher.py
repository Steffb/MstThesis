...
for i in range(3,15):
	...
	exec_ssh_command(name_to_sshInfo['p1'], 'ovs-ofctl mod-flows foo in_port=1,actions:output=%d'%(i))
	exec_ssh_command(name_to_sshInfo['p2'], 'ovs-ofctl mod-flows foo in_port=1,actions:output=%d'%(i))


	print '%s : Sucsessfully changed flow to %d'%(unixtime,i)
