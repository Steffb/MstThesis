...
for i in range(20):
	...
	exec_ssh_command(name_to_sshInfo['p1'], 'ovs-vsctl add-port foo '+newPort+' -- set interface '+newPort+' type=ipsec_gre options:remote_ip=192.168.1.122 options:key='+str(66+i)+' options:psk=%s'%(newkey))
	#exec_ssh_command(name_to_sshInfo['p2'], 'ovs-vsctl add-port foo '+newPort+' -- set interface '+newPort+' type=ipsec_gre options:remote_ip=192.168.1.146 options:key='+str(i)+' options:psk=%s'%(newkey))
